...
import { fmtBRL, fmtPct, fmtNum } from '@/lib/formatters';
import { BarChart, Bar, LineChart, Line, Area, AreaChart, PieChart, Pie, Cell, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, ReferenceLine, RadarChart, Radar, PolarGrid, PolarAngleAxis } from 'recharts';
import { AlertTriangle, CheckCircle, TrendingUp, TrendingDown, Printer } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <svg width="120" height="120" viewBox="0 0 100 100">
        ...
        <text x="50" y="48" textAnchor="middle" fill={color} className="font-display text-2xl" fontSize="24" fontWeight="700">{score}</text>
        <text x="50" y="64" textAnchor="middle" fill="hsl(220,15%,55%)" fontSize="9">pontos</text>
      </svg>
    </div>
  );
}
...
export default function DashboardPage({ resultado }: DashboardProps) {
  ...
  const fluxoAcum = c.fluxosCaixa.reduce<{ mes: number; valor: number; fase: string }[]>((acc, fc, i) => {
  ...
  // Comparáveis bar
  const compData = [
    ...inputs.comparaveis.filter(c => c.area > 0).map(c => ({ name: c.descricao.substring(0, 20), value: c.preco / c.area })),
    { name: inputs.nome, value: vgvEfetivo / inputs.areaConstruida }
  ];

  // Cenários comparativo
  const cenarioComp = [
    { indicador: 'Lucro Líq.', A: resultado.cenarios.A.lucroLiquido, B: resultado.cenarios.B.lucroLiquido, C: resultado.cenarios.C.lucroLiquido },
    { indicador: 'ROE', A: resultado.cenarios.A.roe, B: resultado.cenarios.B.roe, C: resultado.cenarios.C.roe },
    { indicador: 'Margem', A: resultado.cenarios.A.margem, B: resultado.cenarios.B.margem, C: resultado.cenarios.C.margem },
  ];
  const cenBarData = [
  ...
  // Sensitivity: ROE × (ΔVGV vs ΔCusto)
  const deltas = [-0.10, -0.05, 0, 0.05, 0.10];
  const sensROE = deltas.map(dVGV => {
    const row: Record<string, number | string> = { deltaVGV: `${dVGV >= 0 ? '+' : ''}${(dVGV * 100).toFixed(0)}%` };
    deltas.forEach(dCusto => {
      const modInputs = { ...inputs, valorVenda: inputs.valorVenda * (1 + dVGV), custoPorM2: inputs.custoPorM2 * (1 + dCusto) };
      const { calcularProjetoCompleto: calc } = require('@/lib/calculos');
      const r = calc(modInputs);
      row[`${(dCusto * 100).toFixed(0)}%`] = r.cenarios[cenarioSel].roe;
    });
    return row;
  });

  // Sensitivity: Margem × (ΔVGV vs Meses Pós)
  const mesesOptions = [1, 3, 6, 9, 12];
  const sensMargem = deltas.map(dVGV => {
    const row: Record<string, number | string> = { deltaVGV: `${dVGV >= 0 ? '+' : ''}${(dVGV * 100).toFixed(0)}%` };
    mesesOptions.forEach(m => {
      const modInputs = { ...inputs, valorVenda: inputs.valorVenda * (1 + dVGV), mesesPosB: m, mesesPosA: m, mesesPosC: m };
      const { calcularProjetoCompleto: calc } = require('@/lib/calculos');
      const r = calc(modInputs);
  ...
  const checksOK = auditChecks.filter(c => c.status === 'ok').length;
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="w-full space-y-2">
            {[
              ...
              <div key={b.label} className="space-y-0.5">
                <div className="flex justify-between text-[10px] text-muted-foreground"><span>{b.label}</span><span>{Math.round(b.val)}%</span></div>
      ...
      {/* BLOCO 3 — Comparativo de Cenários */}
        ...
        <div className="bg-card border border-border rounded-lg p-5 overflow-x-auto">
          ...
          <table className="w-full text-sm">
            <thead><tr className="border-b border-border text-muted-foreground text-xs">
              <th className="text-left py-2">Indicador</th><th className="text-right py-2">Cenário A</th><th className="text-right py-2">Cenário B</th><th className="text-right py-2">Cenário C</th>
            </tr></thead>
            <tbody className="font-mono">
              <tr className="border-b border-border/50"><td className="py-2">Lucro Líquido</td><td className="text-right">{fmtBRL(resultado.cenarios.A.lucroLiquido)}</td><td className="text-right">{fmtBRL(resultado.cenarios.B.lucroLiquido)}</td><td className="text-right">{fmtBRL(resultado.cenarios.C.lucroLiquido)}</td></tr>
              <tr className="border-b border-border/50"><td className="py-2">ROE</td><td className="text-right">{fmtPct(resultado.cenarios.A.roe)}</td><td className="text-right">{fmtPct(resultado.cenarios.B.roe)}</td><td className="text-right">{fmtPct(resultado.cenarios.C.roe)}</td></tr>
              <tr className="border-b border-border/50"><td className="py-2">Margem</td><td className="text-right">{fmtPct(resultado.cenarios.A.margem)}</td><td className="text-right">{fmtPct(resultado.cenarios.B.margem)}</td><td className="text-right">{fmtPct(resultado.cenarios.C.margem)}</td></tr>
              <tr className="border-b border-border/50"><td className="py-2">Exposição</td><td className="text-right">{fmtBRL(resultado.cenarios.A.exposicaoMaxima)}</td><td className="text-right">{fmtBRL(resultado.cenarios.B.exposicaoMaxima)}</td><td className="text-right">{fmtBRL(resultado.cenarios.C.exposicaoMaxima)}</td></tr>
              <tr className="border-b border-border/50"><td className="py-2">Custo Pós-Obra</td><td className="text-right">{fmtBRL(resultado.cenarios.A.totalPos)}</td><td className="text-right">{fmtBRL(resultado.cenarios.B.totalPos)}</td><td className="text-right">{fmtBRL(resultado.cenarios.C.totalPos)}</td></tr>
              <tr><td className="py-2">Duração Total</td><td className="text-right">{resultado.cenarios.A.duracaoTotal}m</td><td className="text-right">{resultado.cenarios.B.duracaoTotal}m</td><td className="text-right">{resultado.cenarios.C.duracaoTotal}m</td></tr>
            </tbody>
          </table>
        </div>
        <div className="bg-card border border-border rounded-lg p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={cenBarData} barGap={4}>
              ...
              <YAxis tick={{ fill: '#8892A8', fontSize: 11 }} tickFormatter={v => `${v}%`} />
              <Tooltip contentStyle={{ background: CARD_BG, border: '1px solid hsl(225,25%,25%)', borderRadius: 8, fontSize: 12 }} />
              <Bar dataKey="A" fill={GREEN} radius={[4, 4, 0, 0]} />
              <Bar dataKey="B" fill={GOLD} radius={[4, 4, 0, 0]} />
              <Bar dataKey="C" fill={RED} radius={[4, 4, 0, 0]} />
      ...
      <div className="bg-card border border-border rounded-lg p-5">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={fluxoAcum}>
            ...
            <CartesianGrid strokeDasharray="3 3" stroke="hsl(225,25%,20%)" />
            <XAxis dataKey="mes" tick={{ fill: '#8892A8', fontSize: 11 }} label={{ value: 'Meses', position: 'insideBottomRight', fill: '#8892A8', fontSize: 10 }} />
            <YAxis tick={{ fill: '#8892A8', fontSize: 11 }} tickFormatter={v => `${(v / 1000).toFixed(0)}k`} />
            <Tooltip contentStyle={{ background: CARD_BG, border: '1px solid hsl(225,25%,25%)', borderRadius: 8, fontSize: 12 }}
              formatter={(v: number) => [fmtBRL(v), 'Acumulado']}
              labelFormatter={(l) => {
                const pt = fluxoAcum[l as number];
                return pt ? `Mês ${l} — ${pt.fase}` : `Mês ${l}`;
              }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-card border border-border rounded-lg p-5 overflow-x-auto">
          <h2 className="font-display text-lg mb-3">Detalhamento de Custos</h2>
          <table className="w-full text-sm">
              ...
              {donutData.map((d, i) => {
                ...
                return (
                  <tr key={i} className="border-b border-border/50">
                    <td className="py-2 flex items-center gap-2"><span className="w-2 h-2 rounded-full" style={{ background: donutColors[i] }} />{d.name}</td>
      ...
      {/* BLOCO 6 — Análise de Mercado */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-card border border-border rounded-lg p-5">
          <h2 className="font-display text-lg mb-3">Comparáveis de Mercado (R$/m²)</h2>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={compData} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(225,25%,20%)" />
              <XAxis type="number" tick={{ fill: '#8892A8', fontSize: 11 }} tickFormatter={v => `${(v / 1000).toFixed(0)}k`} />
              <YAxis type="category" dataKey="name" width={100} tick={{ fill: '#8892A8', fontSize: 10 }} />
              <Tooltip contentStyle={{ background: CARD_BG, border: '1px solid hsl(225,25%,25%)', borderRadius: 8, fontSize: 12 }}
              ...
              <ReferenceLine x={mercado.medianaComps} stroke={YELLOW} strokeDasharray="4 4" label={{ value: 'Mediana', fill: YELLOW, fontSize: 10 }} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-card border border-border rounded-lg p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-muted/30 rounded-lg p-3"><p className="text-[10px] text-muted-foreground">Mediana R$/m²</p><p className="font-mono text-sm">{fmtBRL(mercado.medianaComps)}</p></div>
            <div className="bg-muted/30 rounded-lg p-3"><p className="text-[10px] text-muted-foreground">Média R$/m²</p><p className="font-mono text-sm">{fmtBRL(mercado.mediaComps)}</p></div>
            <div className="bg-muted/30 rounded-lg p-3"><p className="text-[10px] text-muted-foreground">Desvio vs modelo</p><p className="font-mono text-sm">{fmtPct(mercado.desvioPreco)}</p></div>
            <div className="bg-muted/30 rounded-lg p-3"><p className="text-[10px] text-muted-foreground">Break-even buffer</p><p className="font-mono text-sm">{fmtPct(bufferBE)}</p></div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-card border border-border rounded-lg p-4 overflow-x-auto">
            ...
            <table className="w-full text-[11px]">
              <thead><tr className="text-muted-foreground"><th className="py-1">ΔVGV \ ΔCusto</th>{deltas.map(d => <th key={d} className="py-1 text-center">{d >= 0 ? '+' : ''}{(d * 100).toFixed(0)}%</th>)}</tr></thead>
              <tbody className="font-mono">
                {sensROE.map((row, i) => (
                  <tr key={i}>
                    <td className="py-1 text-muted-foreground">{row.deltaVGV as string}</td>
                    {deltas.map(d => {
                      const v = row[`${(d * 100).toFixed(0)}%`] as number;
                      return <td key={d} className={`py-1 text-center rounded ${cellColor(v, [0.6, 0.3])}`}>{(v * 100).toFixed(0)}%</td>;
                    })}
                  </tr>
                ))}
              </tbody>
          ...
          {/* Margem × ΔVGV vs Meses Pós */}
          <div className="bg-card border border-border rounded-lg p-4 overflow-x-auto">
            <p className="text-xs text-muted-foreground mb-2 font-medium">Margem × (ΔVGV vs Meses Pós)</p>
            <table className="w-full text-[11px]">
              <thead><tr className="text-muted-foreground"><th className="py-1">ΔVGV \ Meses</th>{mesesOptions.map(m => <th key={m} className="py-1 text-center">{m}m</th>)}</tr></thead>
              <tbody className="font-mono">
                {sensMargem.map((row, i) => (
                  <tr key={i}>
                    <td className="py-1 text-muted-foreground">{row.deltaVGV as string}</td>
                    {mesesOptions.map(m => {
                      const v = row[`${m}m`] as number;
                      return <td key={m} className={`py-1 text-center rounded ${cellColor(v, [0.2, 0.1])}`}>{(v * 100).toFixed(0)}%</td>;
                    })}
                  </tr>
                ))}
              </tbody>
          ...
          {/* Exposição × ΔTaxa vs LTV */}
          <div className="bg-card border border-border rounded-lg p-4 overflow-x-auto">
            <p className="text-xs text-muted-foreground mb-2 font-medium">Exposição × (ΔTaxa vs LTV)</p>
            <table className="w-full text-[11px]">
              <thead><tr className="text-muted-foreground"><th className="py-1">ΔTaxa \ LTV</th>{ltvOptions.map(l => <th key={l} className="py-1 text-center">{(l * 100).toFixed(0)}%</th>)}</tr></thead>
              <tbody className="font-mono">
                {sensExposicao.map((row, i) => (
                  <tr key={i}>
                    <td className="py-1 text-muted-foreground">{row.deltaTaxa as string}</td>
                    {ltvOptions.map(l => {
                      const v = row[`${(l * 100).toFixed(0)}%`] as number;
                      return <td key={l} className={`py-1 text-center rounded ${cellColorExp(v)}`}>{(v / 1000).toFixed(0)}k</td>;
                    })}
                  </tr>
                ))}
              </tbody>
      ...
      <div className="bg-card border border-border rounded-lg p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-sm font-mono text-muted-foreground">{checksOK}/{auditChecks.length} checks OK ({((checksOK / auditChecks.length) * 100).toFixed(0)}%)</span>
        </div>
